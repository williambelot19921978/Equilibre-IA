...
function contextWithKnowledge(): AssistantConversationContext {
  return {
    ...
    lifeKnowledge: {
      ...
      timelineCount: 1,
      phrasingHints: ["Comme tu préfères généralement courir le matin…"],
      topConfirmedLabels: ["Course matinale: Tu cours généralement le matin"],
    },
...
describe("Conversation — Life Knowledge phrasing", () => {
  it("utilise uniquement les préférences confirmées dans le ton", () => {
    ...
    expect(response.text).toContain("matin");
  });
});
